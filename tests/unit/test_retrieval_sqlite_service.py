...
def test_query_kb_contract_for_three_modes(tmp_path):
        ...
        [
            {
                ...
                "year": "2024",
                "journal": "J1",
                "abstract": "BRCA1 in breast cancer",
            }
        ]
    )
    ...
    assert by_pmid["results"][0]["normalized_id"] == "HGNC:1100"

    by_norm = query_kb(mode="normalized_id", normalized_id="HGNC:1100", db_path=db_path)
    assert by_norm["mode"] == "normalized_id"
    assert by_norm["filters"] == {"normalized_id": "HGNC:1100"}
    ...
    assert by_type_kw["filters"] == {"entity_type": "Gene", "keyword": "brca"}
